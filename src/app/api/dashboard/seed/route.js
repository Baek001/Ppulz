  ...
  try {
    ...
    if (existingError) {
      if (isMissingTableError(existingError, 'seed_requests')) {
        return NextResponse.json({ queued: false, reason: 'queue_unavailable' });
      }
      return NextResponse.json(
        { queued: false, reason: 'queue_failed', error: existingError.message },
        { status: 500 },
      );
    }
    ...
    if (upsertError) {
      return NextResponse.json(
        { queued: false, reason: 'queue_failed', error: upsertError.message },
        { status: 500 },
      );
    }
    ...
    return NextResponse.json(
      { error: error?.message ?? 'Failed to queue seed.' },
      { status: 500 },
    );
  }
}
